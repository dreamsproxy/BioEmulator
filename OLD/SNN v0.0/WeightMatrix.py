...
class WeightMatrix:
    def __init__(self, num_neurons, pre_alpha, post_alpha, init_type='glorot_uniform'):
        ...
        self.embedding_weights = np.random.uniform(0.25, 0.75, (self.num_neurons, 2))
    ...
    def update_encodings(self, encoding, spikes):
        ...
        self.embedding_weights[enc_increase] += self.a_post
        self.embedding_weights[enc_decrease] -= self.a_pre
        self.embedding_weights = np.clip(self.embedding_weights, 0.0, 1.0)
        self.embedding_weights = (self.embedding_weights - self.embedding_weights.min()) / (self.embedding_weights.max() - self.embedding_weights.min())
