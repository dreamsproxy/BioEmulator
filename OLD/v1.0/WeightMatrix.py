...
class WeightMatrix(object):
    def __init__(self, num_neurons:int, pre_alpha:np.float64, post_alpha:np.float64, init_type:str='glorot_uniform'):
        self.a_pre = np.float64(pre_alpha)
        self.a_post = np.float64(post_alpha)
        self.num_neurons:int = num_neurons
        self.weights:np.ndarray = self._initialize_weights(init_type)
        self.hebb_weights:np.ndarray = np.random.uniform(0.0, 1.0, (self.num_neurons, self.num_neurons)).astype(np.float64)
        self.embedding_weights:np.ndarray = np.random.uniform(0.25, 0.75, (self.num_neurons, 2)).astype(np.float64)

    def _initialize_weights(self, init_type:str='glorot_uniform') -> np.ndarray:
        if 'glorot_normal'not in init_type:
